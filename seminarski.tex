...
%\author{Bogićević Marica, Karanović Boris, Košanin Petar, Šašić Filip\\ marica.bogicevic8@gmail.com, boskonet@gmail.com, kosaninp@gmail.com, sasicf@gmail.com}
...
\Else{ 
deltaE=nFitness-prevFitness\;  treshold = $e^{-k*\frac{deltaE}{temperature}}$\;
\If{(random(0,1)<treshold)}  {Accept configuration\;
...
\begin{algorithm}[H]
\SetAlgoLined
\KwInput{Maximum number of generations: $G_{max}$ ; Population size: $S_{pop}$ ; Size of mating pool: $S_{pool}$ ; Tournament size: $S_{tour}$ ; Crossover probability: $P_{c}$ ; Mutation probability: $P_{m}$.}
\textbf{P} := GenerateInitialPopulation($S_{pop}$)
\;
 \Repeat{TerminationCriterion($G_{max}$)}{
    $\textbf{P}_{parent}$ := Selection($P$, $S_{pool}$, $S_{tour}$)\;
    $\textbf{P}_{child}$ := GeneticOperation($P_{parent}$, $P_{c}$, $P_{m}$)\;
    $\textbf{P}_{new}$ := LocalSearch($P_{child}$)\;
    $\textbf{P}$ := UpdatePopulation($P$, $P_{new}$)\;
 }
 \KwOutput{Convert the fittest chromosome in \textbf{P} into a partition solution and output.}
